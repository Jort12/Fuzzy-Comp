...
from rl_policy_exp import SharedActorPolicy
...
#Find the highest-scoring threat based on a combination of distance, time-to-collision, closing speed, and size.
...
class RLController(KesslerController):
    ...
    def actions(self, ship_state, game_state):
        ...
        # One forward pass for all actions
        if self.deterministic:
            with torch.no_grad():
                means, _, fire_logit, mine_logit = self.actor(xb, sc_oh)
                ...
                # Dummy values for trajectory (not used during eval)
            ...
            action, raw_sample, fire_a, mine_a, log_prob = self.actor.get_action(xb, sc_oh)
            thrust_norm = action[0, 0].item()
            turn_norm = action[0, 1].item()
            fire = bool(fire_a.item())
            mine = bool(mine_a.item())

        # Post-sampling gains (environment interface, not policy distribution)
        ...
        self._pending = {
            "features": xb.detach(),
            "raw_sample_m": raw_sample.detach(),
            "fire_action": fire_a.detach(),
            "mine_action": mine_a.detach(),
            "log_prob": log_prob.detach().squeeze(),
            "reward": 0.0,
        }
