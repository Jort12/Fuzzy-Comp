...
def _get_pos(obj):
    """Extract (x,y) from an object under common attribute names."""
    p = _get(obj, ["position", "pos"], None)
...
def _get_vel(obj):
    """Extract (vx,vy) from an object under common attribute names."""
    v = _get(obj, ["velocity", "vel"], (0.0, 0.0)) or (0.0, 0.0)
    try:
...
class AggressiveFuzzyController(KesslerController):
    ...
    def _build_fis(self):
        ...
        thrust = ctrl.Consequent(np.linspace(0.0, 100.0, 101), "thrust")
        ...
        thrust['reverse'] = fuzz.trimf(thrust.universe, [0, 0, 20])
        thrust['medium']  = fuzz.trimf(thrust.universe, [60, 72, 84])
        thrust['high']    = fuzz.trimf(thrust.universe, [80, 100, 100])
        ...
        rules += [
            ctrl.Rule(mine_distance['very_near'] & mine_angle['left'],
                      (thrust['high'], turn['hard_right'], fire['no'], mine['no'])),
            ctrl.Rule(mine_distance['very_near'] & mine_angle['right'],
                      (thrust['high'], turn['hard_left'],  fire['no'], mine['no'])),
        ...
        rules += [
            ctrl.Rule(mine_distance['near'] & mine_angle['left'],
                      (thrust['high'], turn['hard_right'], mine['no'])),
            ctrl.Rule(mine_distance['near'] & mine_angle['right'],
                      (thrust['high'], turn['hard_left'],  mine['no'])),
            ctrl.Rule(mine_distance['near'] & mine_angle['ahead'],
                      (thrust['high'], turn['soft_right'], mine['no'])),
            ctrl.Rule(mine_distance['near'] & angle['ahead'], (fire['yes'])),
        ]

        rules += [
            ctrl.Rule(mine_distance['mid'] & mine_angle['left'],
                      (thrust['medium'], turn['soft_right'])),
            ctrl.Rule(mine_distance['mid'] & mine_angle['right'],
                      (thrust['medium'], turn['soft_left'])),
            ctrl.Rule(mine_distance['mid'] & angle['ahead'], (fire['yes'])),
        ]


        rules += [
            ctrl.Rule(danger['imminent'] & angle['left'],
                      (thrust['reverse'], turn['hard_right'], fire['no'], mine['no'])),
            ctrl.Rule(danger['imminent'] & angle['right'],
                      (thrust['reverse'], turn['hard_left'],  fire['no'], mine['no'])),
            ctrl.Rule(danger['imminent'] & angle['ahead'],
                      (thrust['reverse'], turn['soft_right'], fire['no'], mine['no'])),
            ctrl.Rule(danger['risky'], (thrust['medium'])),
        ]

        rules += [
            ctrl.Rule((angle['ahead']) & (danger['safe'] | danger['risky']) &
                      (mine_distance['far'] | mine_distance['mid'] | mine_distance['near']),
                      fire['yes'])
        ]

        rules.append(ctrl.Rule(mine_distance['far'] & distance['very_close'],
                               (thrust['reverse'], turn['zero'], fire['no'])))

        rules += [
            ctrl.Rule(mine_distance['far'] & (danger['safe'] | danger['risky']) & distance['close'] & angle['ahead'],
                      (thrust['medium'], turn['zero'], fire['yes'])),
            ctrl.Rule(mine_distance['far'] & (danger['safe'] | danger['risky']) & distance['close'] & angle['left'],
                      (thrust['medium'], turn['soft_left'])),
            ctrl.Rule(mine_distance['far'] & (danger['safe'] | danger['risky']) & distance['close'] & angle['right'],
                      (thrust['medium'], turn['soft_right'])),
        ]

        rules += [
            ctrl.Rule(mine_distance['far'] & danger['safe'] & distance['sweet'] & angle['ahead'],
                      (thrust['medium'], turn['zero'], fire['yes'])),
            ctrl.Rule(mine_distance['far'] & danger['safe'] & distance['sweet'] & (angle['left'] | angle['right']),
                      (thrust['medium'], fire['yes'])),
        ]

        rules += [
            ctrl.Rule(mine_distance['far'] & (danger['safe'] | danger['risky']) & distance['far'] & angle['ahead'],
                      (thrust['high'], turn['zero'], fire['yes'])),
            ctrl.Rule(mine_distance['far'] & (danger['safe'] | danger['risky']) & distance['far'] & angle['left'],
                      (thrust['high'], turn['soft_left'])),
            ctrl.Rule(mine_distance['far'] & (danger['safe'] | danger['risky']) & distance['far'] & angle['right'],
                      (thrust['high'], turn['soft_right'])),
        ]

        rules.append(ctrl.Rule(mine_distance['far'] & (distance['sweet'] | distance['far']) &
                               rel_speed['fast'] & angle['ahead'], (fire['yes'])))


        rules += [
            ctrl.Rule(mine_distance['far'] & danger['safe'] &
                      (distance['close'] | distance['sweet']) & angle['ahead'], (mine['yes'])),
            ctrl.Rule(mine_distance['far'] & (distance['close'] | distance['sweet']) &
                      rel_speed['fast'], (mine['yes'])),
    ...
    def _norm_distance(d, map_diag):
        """Map real distance to [0..1]; 0 = near, 1 = far. Uses half map diagonal as 'far' scale."""
    ...
    def _norm_rel_speed(v_rel, max_approach=500.0):
        """Map line-of-sight approach speed to [0..1]; ignore recession (clip at 0)."""
        v = max(0.0, min(max_approach, v_rel))
        return v / max_approach

    @staticmethod
    def _norm_angle_deg_to_unit(a_deg):
        """Map [-180..180] degrees to [-1..1] for angle membership."""
        return max(-1.0, min(1.0, a_deg / 180.0))

    @staticmethod
    def _norm_mine_distance(d, scale=320.0):
        """Map distance to nearest mine into [0..1]; larger scale => we treat mines as 'near' from farther away."""
        return max(0.0, min(1.0, d / scale))

    @staticmethod
    def _norm_ttc_like(dist_n, rel_speed_n):
        """
        Time-to-collision-like safety proxy in [0..1]:
        - Combine proximity (1 - distance) and approach speed to estimate risk.
        - Return 0 for 'imminent' and 1 for 'safe' (we invert the risk).
        """
    ...
    def actions(self, ship_state, game_state):
        ...
        Returns:
            thrust (float), turn_rate (float deg/s), fire (bool), drop_mine (bool)
        """
        ...
        out_thrust = float(sim.output.get('thrust', 75.0))
        out_turn   = float(sim.output.get('turn', 0.0))
        out_fire   = float(sim.output.get('fire', 1.0))
        out_mine   = float(sim.output.get('mine', 0.0))

        T_MAX = 230.0
        engine_thrust = ((out_thrust / 100.0) * 1.4 - 0.2)
        engine_thrust = max(-1.0, min(1.0, engine_thrust)) * T_MAX
